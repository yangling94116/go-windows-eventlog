...
func newRecordFilter(q query) (*recordFilter, error) {
	f := &recordFilter{ignoreOlder: q.IgnoreOlder}
...
func (f *recordFilter) match(r *Record) bool {
	...
	if f.ignoreOlder > 0 && !r.TimeCreated.SystemTime.IsZero() && time.Since(r.TimeCreated.SystemTime) > f.ignoreOlder {
		return false
	}
	...
	for _, included := range f.includeIDs {
		if included.contains(eventID) {
			return true
		}
	}
	return false
...
func parseEventIDRange(expression, original string) (eventIDRange, error) {
	parts := strings.Split(expression, "-")
	switch len(parts) {
	case 1:
		value, err := parseEventID(parts[0], original)
		if err != nil {
			return eventIDRange{}, err
		}
		return eventIDRange{start: value, end: value}, nil
	case 2:
