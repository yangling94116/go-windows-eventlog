...
func UnmarshalXML(rawXML []byte) (Event, error) {
	var event Event
	decoder := xml.NewDecoder(NewSafeReader(rawXML))
	decoder.Strict = false
	decoder.Entity = xml.HTMLEntity // Prevent XXE attacks
...
func (e Event) Fields() map[string]interface{} {
	...
	if e.KeywordsRaw&keywordAuditFailure > 0 {
		setNestedField(win, "outcome", "failure")
	} else if e.KeywordsRaw&keywordAuditSuccess > 0 {
		setNestedField(win, "outcome", "success")
	}
