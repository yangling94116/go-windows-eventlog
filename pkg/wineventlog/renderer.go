...
func NewRenderer(locale uint32, session EvtHandle) (*Renderer, error) {
	...
	return &Renderer{
		metadataCache: newPublisherMetadataCache(session, locale),
...
func (r *Renderer) renderUser(mds *PublisherMetadataStore, handle EvtHandle, event *winevent.Event) (values []interface{}, fingerprint uint64, err error) {
	...
	for i := 0; i < propertyCount; i++ {
		...
		if err != nil {
			// Failed to read event/user data value. Using nil.
		}
...
func (r *Renderer) addEventData(evtMeta *EventMetadata, values []interface{}, event *winevent.Event) {
	...
	if evtMeta == nil {
		...
		// The number of event data parameters doesn't match the number of parameters in the template.
	}
...
func NewXMLRenderer(locale uint32, isForwarded bool, session EvtHandle) *XMLRenderer {
