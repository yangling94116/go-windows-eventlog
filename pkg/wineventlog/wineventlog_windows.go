...
// utf16BytesToWriter converts UTF-16 bytes to UTF-8 and writes to an io.Writer.
// This replaces libbeat's common.UTF16ToUTF8Bytes function.
func utf16BytesToWriter(b []byte, w io.Writer) error {
	s, err := sys.UTF16BytesToString(b)
	if err != nil {
		return err
	}
...
func FormatEventString(
	...
	if err != nil && !errors.Is(err, windows.ERROR_INSUFFICIENT_BUFFER) {
		...
		if wcharBufferUsed <= wcharBufferSize {
			return utf16BytesToWriter(renderBuf[:wcharBufferUsed*2], out)
		}
	}
	...
	return utf16BytesToWriter(bb.Bytes(), out)
}
...
func renderXML(eventHandle EvtHandle, flag EvtRenderFlag, renderBuf []byte, out io.Writer) error {
	...
	if err != nil && !errors.Is(err, windows.ERROR_INSUFFICIENT_BUFFER) {
		...
		if bufferUsed <= bufferSize {
			return utf16BytesToWriter(renderBuf[:bufferUsed], out)
		}
	}
