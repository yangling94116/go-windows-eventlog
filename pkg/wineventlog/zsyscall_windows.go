...
func systemTimeToFileTime(systemTime *windows.Systemtime, fileTime *windows.Filetime) (err error) {
	r1, _, e1 := syscall.Syscall(procSystemTimeToFileTime.Addr(), 2, uintptr(unsafe.Pointer(systemTime)), uintptr(unsafe.Pointer(fileTime)), 0)
	if r1 == 0 {
		err = errnoErr(e1)
	}
	return
