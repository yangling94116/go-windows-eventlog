...
func (c *Checkpoint) persist() bool {
	...
	err := c.flush()
	if err != nil {
		// Note: logging removed to keep package independent
		// Original code: logp.Err("%v", err)
		return false
	}

	// Note: debug logging removed to keep package independent
	// Original code: logp.Debug("checkpoint", "Checkpoint saved to disk. numUpdates=%d", c.numUpdates)
	c.numUpdates = 0
	return true
}
...
func (c *Checkpoint) createDir() error {
	...
	// Note: logging removed to keep package independent
	// Original code: logp.Info("Creating %s if it does not exist.", dir)
